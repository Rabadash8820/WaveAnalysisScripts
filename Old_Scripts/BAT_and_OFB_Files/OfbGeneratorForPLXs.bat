...
ECHO that contains subdirectories with PLX files.
...
ECHO.
FOR /d /r %%D IN (*) DO (
	REM If this subdirectory contains at least one .plx file...
	IF EXIST "%%~fD\*.plx" (	
		REM Create an .ofb file named like "count_subDirectoryName.ofb"		
		REM Add the line that queues all .plx files in the rootPath directory
		> "!destPath!\!count!_%%~nD.ofb" (
			ECHO // Queue all .PLX files in the directory
			ECHO Dir %%~fD\*.plx
			ECHO(
		)
