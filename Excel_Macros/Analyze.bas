...
Public Sub processRetinalWorkbook(ByVal wbName As String, ByRef numRecordings As Integer)
    ...
    Set summaryTbl = wb.Worksheets(CONTENTS_SHEET_NAME).ListObjects("SummaryTbl")
...
Private Sub addAllAvgsSheet(ByVal retinaName As String, ByRef unitNames As Variant)
    ...
    Worksheets.Add After:=Sheets(CONTENTS_SHEET_NAME)
    ActiveSheet.name = ALL_AVGS_SHEET_NAME
    Set avgsRng = Worksheets(ALL_AVGS_SHEET_NAME).Cells(2, 2)
    ...
    Worksheets(ALL_AVGS_SHEET_NAME).ListObjects.Add( _
        xlSrcRange, _
...
Private Sub addBurstAvgsSheet(ByVal retinaName As String, ByRef unitNames As Variant)
    ...
    'Add the Averages sheet, write out row (unit) headers, and add formatting
    Dim avgsRng As Range
    Worksheets.Add After:=Sheets(ALL_AVGS_SHEET_NAME)
    ActiveSheet.name = BURST_AVGS_SHEET_NAME
    Set avgsRng = Worksheets(BURST_AVGS_SHEET_NAME).Cells(2, 2)
    Cells(1, 1).value = CELL_STR
    avgsRng.offset(0, -1).Resize(numUnits, 1).value = unitNames
    avgsRng.offset(-1, -1).Resize(numUnits + 1).Font.Bold = True
    ...
    'Make a table for all the average values
    Worksheets(BURST_AVGS_SHEET_NAME).ListObjects.Add( _
        xlSrcRange, _
        avgsRng.CurrentRegion, , _
        xlYes) _
...
Private Sub addSttcSheet(ByVal retinaName As String, ByRef unitNames As Variant)
    ...
    Worksheets.Add After:=Sheets(BURST_AVGS_SHEET_NAME)
    ActiveSheet.name = STTC_SHEET_NAME
    Set sttcRng = Worksheets(STTC_SHEET_NAME).Cells(4, 1)
    With sttcRng
    ...
    Worksheets(STTC_SHEET_NAME).ListObjects.Add( _
        xlSrcRange, _
...
Private Sub storeSttcValues(ByVal duration As Double, ByVal numUnits As Long)
    ...
    Set outputRng = Worksheets(STTC_SHEET_NAME).Cells(4, 4).Resize(numRows, 1)
...
Private Sub storePreValues(ByVal cellIndex As Integer, ByRef spikes As Variant, ByRef bursts As Variant, ByVal recDuration As Double)
    ...
    Set preCell = Worksheets(ALL_AVGS_SHEET_NAME).Cells(cellIndex + 1, 1 + 1)
...
Private Sub storePostValues(ByVal cellIndex As Integer, ByRef spikes As Variant, ByRef bursts As Variant, ByVal recDuration As Double, ByVal wabRatio As Double)
    ...
    Dim oldResults As Variant
    Set postCell = Worksheets(BURST_AVGS_SHEET_NAME).Cells(cellIndex + 1, 1 + 1)
    ...
    Set pbwCell = Worksheets(ALL_AVGS_SHEET_NAME).Cells(cellIndex + 1, NUM_BKGRD_PROPERTIES + 1)
...
Private Function getSpikeTrain(ByVal spikeCol As Integer) As Variant
    ...
    numSpikes = IIf(numSpikes = 1048576 - 1, 0, numSpikes)
    If numSpikes = 0 Then
...
Private Function getBurstTrain(ByVal spikeCol As Integer, ByVal numUnits As Integer) As Variant
    ...
    numBursts = IIf(numBursts = 1048576 - 1, 0, numBursts)
    If numBursts = 0 Then
...
Private Sub reduceToAvgs(ByVal numRecordings As Integer, ByVal numUnits As Integer)
    ...
    Set data = Worksheets(ALL_AVGS_SHEET_NAME).Cells(2, 2).Resize(numUnits, NUM_PROPERTIES)
    averages = data.value
    For r = 1 To numUnits
    ...
    Set data = Worksheets(STTC_SHEET_NAME).Cells(3, 2).Resize(numUnits, numUnits)
    sttcs = data.value
    For r = 1 To numUnits
        For c = 1 To numUnits
...
Public Sub finalize(ByVal numUnits As Integer)
    ...
    With Worksheets(ALL_AVGS_SHEET_NAME)
        .Cells.HorizontalAlignment = xlCenter
        .Cells.VerticalAlignment = xlCenter
    ...
    With Worksheets(STTC_SHEET_NAME)
        .Cells.HorizontalAlignment = xlCenter
        .Cells.VerticalAlignment = xlCenter
    ...
    Worksheets(ALL_AVGS_SHEET_NAME).Activate
End Sub
